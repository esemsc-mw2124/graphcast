...
def extract_input_target_times(
  ...
  # Ensure time is DataArray (probably is)
  if not isinstance(time, xarray.DataArray):
      time = xarray.DataArray(time)

  # Ensure target_duration is DataArray
  if not isinstance(target_duration, xarray.DataArray):
      target_duration = xarray.DataArray(target_duration)

  # Wrap time[-1] so that subtraction broadcasts properly
  time_offset = xarray.DataArray(time[-1])

  # Perform the safe operation
  dataset = dataset.assign_coords(time=time + target_duration - time_offset)
